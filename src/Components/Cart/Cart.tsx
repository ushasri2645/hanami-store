...
const Cart = () => {
    ...
    const calculateItemTotalDiscount= (product:TItem,quantity:number)=>{
        if(product.offer)
            return (product.price * quantity)-(product.price*(product.offer/100));
        else
    ...
    const calculateCartTotalDiscount = () => {
        return cart.reduce((total, cartItem) => {
            ...
            if (product && product.offer) {
                let val = calculateItemTotalDiscount(product, cartItem.quantity);
                return total + val;
            }
            return total;
        }, 0);
                        ...
                        return (
                            <div key={cartItem.id} className={styles.cartItem}>
                                ...
                                <div className={styles.itemDetails}>
                                    <h2>{product.name}</h2>
                                    <p>Price: ${product.price.toFixed(2)} {product.offer?<p>Discount: ${calculateItemTotalDiscount(product,cartItem.quantity).toFixed(2)}</p>:''}</p>
                        ...
                        <div  className={styles.priceOrderSection}>
                            <div className={styles.cartTotal}>
                                ...
                                <div className={styles.priceDetails}>
                                    ...
                                    <div className={styles.subPrice}>
                                        <p>Discount</p>
                                        <p className={styles.green}>{calculateCartTotalDiscount().toFixed(2)}</p>
                                    </div>
                                    
                                    <div className={styles.subPrice}>
                                        <p>Delivery Charges</p>
                                        <p className={styles.green} >FREE</p>
                                    </div>
                                    <hr></hr>
                                    <div className={styles.subPrice}>
                                        <p>Total Amount</p>
                                        <p>{(calculateCartTotal()-calculateCartTotalDiscount()).toFixed(2)}</p>
                                    </div>
                                    <hr></hr>
                                    <p className={styles.green}>You Saved ${calculateCartTotalDiscount().toFixed(2)} on this order</p>
                                </div>
                            ...
                            <div className={styles.orderSection}>
                                <button className={styles.priceBtn}>
                                    <p>${(calculateCartTotal()-calculateCartTotalDiscount()).toFixed(2)}</p>
                                    <p className={styles.fontSmall}>View Price Details</p>
                                </button>
                                <button
